...
class ExportService {
  ...
  Stream<ExportEvent> exportDocuments(
    ...
    _streamSink = exportEventStreamController.sink;
    _httpClient = http.Client();
    _cancelExportRequested = false;
    _canceledBeforeComplete = false;

    unawaited(_exportDocuments(validArgs, results)
        .mapLeft((l) => [l])
        .map((r) => r.setFinished())
        .bimap(
          (l) {
            _streamSink?.add(ExportEvent.exportFinished(
              results: left(l),
  ...
  void cancelExport() {
    ...
    _streamSink?.add(const ExportEvent.cancelExportRequested());
  }
  ...
  TaskEither<ExportFailure, ExportResults> _exportDocuments(
    ...
    return TaskEither.tryCatch(() async {
      ...
      _streamSink?.add(ExportEvent.beginExport(
          orgId: v.orgId,
      ...
      final exportResults =
        ...
        (acc, docResult) => acc.map(
          (accResults) => docResult.fold(
            (drL) {
              _streamSink?.add(ExportEvent.docSkippedUnknownFailure(
  ...
  Stream<Either<_DownloadFailure, ExportResults>> _exportDocumentsFromStream(
    ...
    return docStream.flatMap(
      ...
      (doc) async* {
        ...
        _streamSink?.add(ExportEvent.docExportAttempt(
    ...
    return TaskEither.tryCatch(() async {
      ...
      if (File(outFilepath).existsSync()) {
        if (validArgs.copyIfExists) {
          ...
          _streamSink?.add(ExportEvent.docSkippedAlreadyExists(
            docId: doc.documentID,
            n1Path: _getNormalizedN1Path(doc),
            localPath:
      ...
      _streamSink?.add(ExportEvent.docExported(
        docId: doc.documentID,
        n1Path: _getNormalizedN1Path(doc),
        localPath:
